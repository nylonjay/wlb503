package com.bankscene.bes.welllinkbank.view.powerwebview.powerlib;

/**
 * Created by skythinking on 16/3/24.
 */
        ...
        super.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(WebView view, String url) {
                if (isHostnameAllowed(url)) {
                    if(mListener != null) {
                       boolean result =  mListener.onInterceptUrl(url);
                        if(result) {
                            return result;
                        }
                    }
...
//                        super.onUnhandledInputEvent(view, event);
     ...
     * @param fromUrl the URL of the file to download, e.g. the one from `PowerWebView.onDownloadRequested(...)`
    ...
    public static boolean handleDownload(final Context context, final String fromUrl, final String toFilename) {
        ...
        // if the download manager app has been disabled on the device
            ...
            openAppSettings(context, PowerWebView.PACKAGE_NAME_DOWNLOAD_MANAGER);
